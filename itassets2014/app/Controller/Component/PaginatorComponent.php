 ...
 */
	public function validateSort(Model $object, array $options, array $whitelist = array()) {
		if (isset($options['sort'])) {
			...
			if(array_key_exists('order', $options) && is_array($options['order'])) $options['order'] = array_merge($options['order'], array($options['sort'] => $direction));
			else $options['order'] = array($options['sort'] => $direction);
		}
